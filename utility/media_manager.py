...
Enabling Scripts in Tautulli:
 ...
 Examples:
     Find unwatched Movies that were added before 5 days ago and delete
        python media_manager.py --libraries Movies --select unwatched --date "5" --action delete
...
if __name__ == '__main__':
    ...
    if opts.date:
        days = int(opts.date)
    if opts.date:
        opts.date = "" + str(date.today() - timedelta(days)) + ""
    if opts.date:
